...
class GasController extends Controller
{
    ...
    public function getStats(Request $request)
    {
        ...
        if ($period === 'year') {
            ...
            // Calculate starting stock 12 months ago by working backwards from current
            $startingStock = $currentFilledStock;
            for ($i = 11; $i >= 0; $i--) {
                ...
                if ($i > 0) {
                    // Work backwards: current stock came from previous stock + net change
                    $startingStock -= ($purchases - $issues);
                }
            }
            
            // Now work forward from starting stock
            $runningStock = max(0, $startingStock);
            ...
            for ($i = 11; $i >= 0; $i--) {
                ...
                $stats[] = [
                    ...
                    'purchases' => $purchases,
                    'issues' => $issues,
                    'filled_stock' => max(0, $runningStock),
                ];
            }
        } else {
            // Last 30 days
            ...
            // Calculate starting stock 30 days ago by working backwards from current
            $startingStock = $currentFilledStock;
            for ($i = 29; $i >= 0; $i--) {
