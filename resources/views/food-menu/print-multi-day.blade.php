...
<head>
    ...
    <title>Multi-Day Food Menu - Booking #{{ $booking->id }}</title>
    <style>
        @page {
            size: A4 portrait;
            margin: 8mm;
...
<body>
    <div class="page">
        <!-- Header -->
        <div class="header">
            <h1>MULTI-DAY FOOD MENU @if($booking->function_type == 'Wedding')<span class="wedding-tag">WEDDING</span>@endif</h1>
            <div class="date">Booking #{{ $booking->id }}</div>
        </div>
        ...
        @foreach($menusForAllDays as $index => $dayData)
            @php
                $menu = $dayData['menu'];
                $date = $dayData['date'];
            @endphp
            ...
            <!-- Day Separator -->
            <div class="day-separator {{ $index == 0 ? 'first' : '' }}">
                📅 Day {{ $index + 1 }} - {{ $date->format('l, F j, Y') }}
            </div>
