<!-- Day 3 Events Tab Content with Date and Time -->
<h2 class="text-2xl font-bold mb-4">{{ now()->addDays(2)->format('l, F j, Y') }} Functions & Menus</h2>

@if(count($day3Bookings) > 0)
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6" id="day3-events-container">
    @foreach($day3Bookings as $booking)
    <div class="border rounded-lg shadow-md p-4 bg-white">
        ...
        <div class="flex justify-end gap-2 mt-3">
            <a href="{{ route('food-menu.index', ['date' => now()->addDays(2)->format('Y-m-d'), 'booking_id' => $booking->id]) }}" 
               class="bg-blue-600 text-white px-3 py-1 rounded text-sm action-button">
                <i class="fas fa-edit mr-1"></i> Manage Menu
            </a>
            @if($booking->menu)
            <a href="{{ route('food-menu.print', ['booking' => $booking->id, 'date' => now()->addDays(2)->format('Y-m-d')]) }}" 
...
<div class="bg-white rounded-lg shadow-md p-8 text-center">
    ...
    <p class="text-gray-500">There are no scheduled events or functions for {{ now()->addDays(2)->format('l, F j') }}.</p>
</div>
@endif

<!-- Link to food menu generator -->
<div class="mt-6 text-center">
    <a href="{{ route('food-menu.index', ['date' => now()->addDays(2)->format('Y-m-d')]) }}" class="bg-blue-600 text-white px-4 py-2 rounded shadow-md hover:bg-blue-700 transition">
